...
def create_anaglyph_from_stereo(left_path, right_path, output_path):
    ...
    if left is None or right is None:
        print(f"Skipping {left_path} or {right_path}, could not read image.")
        return False
...
def process_anaglyph_batch(batch_tuple, midas_obj, cfg):
    ...
    print(f"> Processing anaglyph batch {batch_idx} frames {start_frame}-{end_frame} ({len(frames)})")
    ...
    for left_file, right_file in zip(left_files, right_files):
        ...
        create_anaglyph_from_stereo(left_path, right_path, anaglyph_path)
